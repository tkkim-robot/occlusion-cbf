...
100 unique indices and no exception rows. The Unicycle runs use the canonical
forward-only bound `v_min=0`.
...
The tuned Unicycle controller profile and the forward-only `v_min=0` robot
bound load automatically:
