...
run_one() {
  ...
  local TLAB=${T//./p}
  local KLAB=${KAPPA//./p}
  local LABEL="T${TLAB}_${MODE}_top${TOPK}_k${KLAB}"
  local OUT="$ROOT_DIR/$LABEL"
  mkdir -p "$OUT"
  ...
  if find "$OUT" -name 'crowd_trials_occlusion_cbf_42_1_100_*.json' | grep -q .; then
    ...
    printf '%s,%s,%s,%s,%s,%s,%s\n' "$LABEL" "$MODE" "$T" "$TOPK" "$KAPPA" "$OUT" "skip_existing" >> "$ROOT_DIR/file_scheduler_manifest.csv"
    return 0
  fi

  printf '%s %s start\n' "$(date '+%F %T')" "$LABEL" >> "$ROOT_DIR/scheduler_progress.log"
  printf '%s,%s,%s,%s,%s,%s,%s\n' "$LABEL" "$MODE" "$T" "$TOPK" "$KAPPA" "$OUT" "started" >> "$ROOT_DIR/file_scheduler_manifest.csv"
  ...
  printf '%s,%s,%s,%s,%s,%s,%s\n' "$LABEL" "$MODE" "$T" "$TOPK" "$KAPPA" "$OUT" "$STATUS" >> "$ROOT_DIR/file_scheduler_manifest.csv"
}
...
wait
printf 'file_scheduler_completed %s\n' "$(date)" >> "$ROOT_DIR/scheduler_progress.log"
