...
Scenario-specific wrappers such as ``examples/test_crowd2.py`` can apply the
...
OCBF_CROWD2_QP_FAILURE_FALLBACK_MODE = "state_safe"
...
CROWD2_ENABLE_VISIBLE_HOCBF_DEFAULT = True
...
def default_visible_hocbf_for_scenario(scenario_label):
    """Visible-obstacle HOCBF is part of the crowd2 paper benchmark default."""
    return CROWD2_ENABLE_VISIBLE_HOCBF_DEFAULT if str(scenario_label).strip().lower() == "crowd2" else False


def apply_crowd2_ocbf_defaults(backup_cbf_overrides):
    """Apply crowd2-specific OCBF defaults without overwriting explicit CLI values."""
    cfg = dict(backup_cbf_overrides or {})
    cfg.setdefault("qp_failure_fallback_mode", OCBF_CROWD2_QP_FAILURE_FALLBACK_MODE)
    return cfg
