...
class OcclusionCBFQP:
    ...
    def __init__(self, robot, robot_spec, num_obs=10, kappa=OCBF_DEFAULT_BARRIER_KAPPA, ax=None):
        ...
        self.dt = self.dt_backup
        self.backup_horizon = self.T_horizon
        self.N = int(self.backup_horizon / self.dt)
        self.alpha = alpha
        ...
        try:
            if hasattr(target, "set_occ_barrier_fn"):
                ...
                # Expected for base robot models without OCC-specific methods.
                pass
    ...
    def _alpha(self, h):
        """Class-K function used by the backup-CBF constraints."""
        return self.alpha * h
    ...
    def _visible_obstacle_barrier(self, robot_state, obs):
        ...
        if row.size >= 8 and callable(dynamic_barrier):
            row = row.copy()
            if not self._is_dynamic_obs_for_di(row):
                row[3:5] = 0.0
            return dynamic_barrier(
                robot_state,
                row,
                self.robot.robot_radius,
                beta=float(self.robot_spec.get("cbf_beta", 1.01)),
            )
        return self.robot.agent_barrier(obs)
    ...
    def _build_di_hocbf_constraint(self, robot_state, obs, mode):
        ...
        h_dot_t = 0.0
        res = self._visible_obstacle_barrier(robot_state, obs)
