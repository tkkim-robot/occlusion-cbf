...
class BaseRobot:
    ...
    def __init__(self, X0, robot_spec, dt, ax):
        ...
        self.robot_spec.setdefault(
            'cam_range',
            self.robot_spec.get('sensing_range', 3.0),
        )
        self.cam_range = float(self.robot_spec['cam_range'])
        ...
        if self.robot_spec['model'] == 'DoubleIntegrator2D':
            ...
            raise ValueError(
                "Supported models are DoubleIntegrator2D, DynamicUnicycle2D, "
                "and Unicycle2D"
            )
