...
class LocalTrackingController:
    def __init__(self, X0, robot_spec,
        ...
        if self.pos_controller_type != 'cbf_qp':
            raise ValueError(
                f"Unsupported base controller: {self.pos_controller_type}. "
                "Project controllers are selected by dynamic_env.main."
            )
        ...
        if self.enable_rotation and self.robot_spec['model'] == 'DoubleIntegrator2D':
            if self.att_controller_type != 'velocity_tracking_yaw':
                raise ValueError(
                    f"Unsupported attitude controller: {self.att_controller_type}"
                )
            from base_control.attitude_control.velocity_tracking_yaw import VelocityTrackingYaw
