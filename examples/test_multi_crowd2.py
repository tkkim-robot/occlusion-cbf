"""
Multi-baseline replay for Crowd2.

This script runs multiple Crowd2 baselines on the same generated scenario and
renders:
...
Run:
    uv run python examples/test_multi_crowd2.py --model di --idx 13
"""
...
def _build_crowd2_scenario(args):
    case_seed = crowd1._compute_case_seed(args.seed, args.idx)
...
def _build_runtime_for_baseline(args, baseline_alias, known_obs, obs_meta, scenario_diag):
    ...
    return crowd1._prepare_crowd_runtime(
...
def run_multi_crowd2(args):
    baselines = list(args.baselines)
    if len(baselines) == 0:
        raise ValueError("At least one baseline must be specified.")

    case_seed, known_obs, obs_meta, scenario_diag = _build_crowd2_scenario(args)
    ...
    if show_animation:
        fig, global_ax, tracking_axes = _make_figure_layout(
            len(baselines),
            width=crowd2.ENV_WIDTH,
            height=crowd2.ENV_HEIGHT,
            known_obs=known_obs,
...
def main():
    parser = argparse.ArgumentParser(description="Run multi-baseline Crowd2 replay.")
    ...
    parser.add_argument("--forced-events", type=int, default=crowd2.DEFAULT_FORCED_EVENTS)
