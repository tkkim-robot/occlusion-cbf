...
# principal hazard. The dense mixed-crowd case already exists as crowd2.
HOME_BG_COUNTS = {
    "single_event": (0, 2),
    "two_event": (1, 3),
}
...
def _route_seg_for_distance(dist: float) -> dict[str, Any]:
    d = float(np.clip(dist, 0.0, crowd2.ROUTE_LENGTH))
    for seg in crowd2.ROUTE_SEGS:
        if d <= float(seg["s1"]) + 1e-9:
            return seg
    return crowd2.ROUTE_SEGS[-1]
...
def _build_event_specs(profile: dict[str, Any], rng: np.random.Generator) -> list[dict[str, Any]]:
    ...
    if family == "single_event":
        ...
        s_event = float(crowd2.ROUTE_LENGTH * rng.uniform(f0, f1))
        return [
            {
                "event_id": 0,
                "s_event": s_event,
    ...
    s_event_1 = float(crowd2.ROUTE_LENGTH * rng.uniform(f0, f1))
    f2_0, f2_1 = single_windows[min(route_bin + 2, len(single_windows) - 1)]
    s_event_2 = float(crowd2.ROUTE_LENGTH * rng.uniform(f2_0, f2_1))
...
def _background_candidate_valid_home(
    ...
    *,
    ...
    radius = float(row[2])
    if crowd1._point_polyline_distance(center, crowd2.ROUTE_XY) < float(min_route_clearance):
        return False
    if float(np.linalg.norm(center - crowd2.ROUTE_XY[0])) < float(start_clearance):
        return False
    if crowd2._rows_overlap(center, radius, existing_rows, margin=0.2):
        return False
...
def _manual_home_event_fallback(
    *,
    ...
    event_xy = crowd2._route_point_at_distance(s_event)
    ...
    for side_try in (float(side_sign), -float(side_sign)):
        for occ_radius in (float(forced_occluder_radius_max), float(forced_occluder_radius_min)):
            occ_xy = crowd2._clip_to_workspace(event_xy + side_try * 1.30 * lateral, margin=1.0)
            occ_guard_radius = float(occ_radius) + 1.0
            if crowd2._rows_overlap(occ_xy, occ_guard_radius, guard_rows, margin=0.5):
                continue
            if crowd2._rows_overlap(occ_xy, float(occ_radius), existing_rows, margin=0.5):
                continue
            observer_xy = crowd2._route_point_at_distance(
                float(np.clip(s_event - 2.3, float(seg["s0"]), float(seg["s1"])))
            )
            occ_dir = crowd1._safe_normalize(occ_xy - observer_xy)
            if occ_dir is None:
                continue
            hidden_radius = 0.34
            hidden_xy = occ_xy + (occ_radius + hidden_radius + 0.18) * occ_dir
            if crowd2._rows_overlap(hidden_xy, hidden_radius, existing_rows, margin=0.16):
                continue
            for target_shift in (
                (-0.20, 0.00),
                (0.00, 0.00),
                ...
                target_xy = crowd2._clip_to_workspace(
                ...
                if bool(forced_validate_occlusion) and not initially_occluded:
                    continue
                pred = crowd2._simulate_route_forced_event_nominal(
                    occluder_xy=occ_xy,
                ...
                if bool(forced_require_corridor_conflict) and not bool(pred["corridor_conflict"]):
                    continue
                hidden_row, hidden_meta = crowd2._apply_outer_flow_bounds(
                    np.array(
                        [hidden_xy[0], hidden_xy[1], hidden_radius, hidden_vel[0], hidden_vel[1], 0.0, crowd2.ENV_HEIGHT, 1.0],
                        dtype=float,
                    ),
                    {
                        "mode": 1,
    ...
    for hidden_radius in hidden_radius_candidates:
        for gap_pad in gap_pad_candidates:
            ...
            for hidden_lat in hidden_lat_candidates:
                ...
                hidden_xy = crowd2._clip_to_workspace(hidden_xy, margin=0.8)
                if crowd2._rows_overlap(hidden_xy, hidden_radius, existing_rows, margin=float(overlap_margin)):
                    continue
                for target_along in target_along_candidates:
                    for target_cross in target_cross_candidates:
                        ...
                        target_xy = crowd2._clip_to_workspace(target_xy, margin=0.5)
                        vel_dir = crowd1._safe_normalize(target_xy - hidden_xy)
                        if vel_dir is None:
                            continue
                        hidden_speed = crowd1._sample_hidden_speed_for_setting(
...
def _make_occluder_row(
    *,
    ...
    away_dir = crowd1._safe_normalize(side_sign * np.asarray([-seg_tangent[1], seg_tangent[0]], dtype=float))
    if away_dir is None:
    ...
    occ_x_max = min(crowd2.ENV_WIDTH - 0.5, float(occ_x_max))
    occ_y_min = max(0.5, float(occ_y_min))
    occ_y_max = min(crowd2.ENV_HEIGHT - 0.5, float(occ_y_max))
    ...
    *,
    ...
    for side_sign in (preferred_side_sign, -preferred_side_sign):
        for s_offset in (0.0, -1.5, 1.5, -3.0, 3.0):
            s_event = float(np.clip(float(spec["s_event"]) + s_offset, 0.08 * crowd2.ROUTE_LENGTH, 0.92 * crowd2.ROUTE_LENGTH))
            seg = _route_seg_for_distance(s_event)
            event_xy = crowd2._route_point_at_distance(s_event)
            for relax in (0.18,):
                    ...
                    for along_jitter in along_candidates:
                        for lead_distance in lead_candidates:
                            for occ_radius in radius_candidates:
                                ...
                                occ_xy = crowd2._clip_to_workspace(occ_xy, margin=1.0)
                                occ_guard_radius = float(occ_radius) + 1.0
                                if crowd2._rows_overlap(occ_xy, occ_guard_radius, guard_rows, margin=0.5):
                                    continue
                                if crowd2._rows_overlap(occ_xy, float(occ_radius), existing_rows, margin=0.5):
                                ...
                                observer_xy = crowd2._route_point_at_distance(observer_dist)
    ...
    # Last-resort fallback: use the crowd2 hidden sampler with shallow attempts.
    for side_sign in (preferred_side_sign, -preferred_side_sign):
        for s_offset in (0.0, -2.5, 2.5):
            s_event = float(np.clip(float(spec["s_event"]) + s_offset, 0.08 * crowd2.ROUTE_LENGTH, 0.92 * crowd2.ROUTE_LENGTH))
            seg = _route_seg_for_distance(s_event)
            ...
            for lateral_mag in lateral_candidates:
                for lead_distance in lead_candidates:
                    for occ_radius in radius_candidates:
                        ...
                        observer_xy = crowd2._route_point_at_distance(observer_dist)
                        hidden = crowd2._sample_hidden_for_event(
                            rng=rng,
                            occ_xy=occ_xy,
                            occ_radius=occ_radius,
                            observer_xy=observer_xy,
    ...
    *,
        ...
        if family == "two_event":
            ...
            if placed is None:
                ...
                for frac in fallback_fracs:
                    placed = _manual_home_event_fallback(
                        s_event=float(frac * crowd2.ROUTE_LENGTH),
                        side_sign=float(spec.get("side_sign", 1.0)),
                        existing_rows=forced_rows,
                        guard_rows=guard_rows,
                        forced_hidden_speed=forced_hidden_speed,
    ...
    if bool(rand_obs) and bg_target > 0:
        bg_v_obs_max, bg_v_obs_min = crowd1._rand_obs_speed_window(
            ...
            rand_obs_setting=rand_obs_setting,
            legacy_speed_max=crowd2.LEGACY_ROUTE_DYN_SPEED_MAX,
        )
        keep_rows: list[np.ndarray] = []
        keep_meta: list[dict[str, Any]] = []
        batch_id = 0
        while len(keep_rows) < bg_target and batch_id < crowd2.ROUTE_BG_BATCH_LIMIT:
            ...
            extra_rows, extra_meta = crowd1.LocalTrackingControllerDyn_OCC.make_random_obstacles7(
                ...
                x_range=(1.0, crowd2.ENV_WIDTH - 1.0),
                y_spawn_range=(1.0, crowd2.ENV_HEIGHT - 1.0),
                r_range=(0.3, 0.4),
                y_bounds=(0.0, crowd2.ENV_HEIGHT),
            ...
            for row, meta in zip(extra_rows, extra_meta):
                ...
                if not _background_candidate_valid_home(
                    ...
                    continue
                row, meta = crowd2._apply_outer_flow_bounds(row, meta)
...
def run_crowd_scenario(
    ...
    forced_occluder_radius_min=crowd2.DEFAULT_FORCED_OCCLUDER_RADIUS_MIN,
    forced_occluder_radius_max=crowd2.DEFAULT_FORCED_OCCLUDER_RADIUS_MAX,
    ...
    if forced_hidden_speed is None:
        forced_hidden_speed = crowd2._resolve_forced_hidden_speed(None, rand_obs_setting)
    else:
        forced_hidden_speed = float(forced_hidden_speed)
        if (
            rand_obs_setting == crowd1.CURRENT_RAND_OBS_SETTING
            ...
            # something else.
            forced_hidden_speed = float(crowd2.DEFAULT_FORCED_HIDDEN_SPEED)
    ...
    return crowd1.run_crowd_scenario(
        ...
        tracking_view_window_size=crowd2.TRACKING_VIEW_WINDOW_SIZE,
...
def main():
    ...
    parser.add_argument("--forced-occluder-radius-min", type=float, default=crowd2.DEFAULT_FORCED_OCCLUDER_RADIUS_MIN)
    parser.add_argument("--forced-occluder-radius-max", type=float, default=crowd2.DEFAULT_FORCED_OCCLUDER_RADIUS_MAX)
    parser.add_argument("--forced-validate-occlusion", type=crowd1._str2bool, nargs="?", const=True, default=True)
