...
`examples/test_crowd_narrow.py`, but changes the mission and obstacle generator:
- workspace: 30 x 30
...
from position_control.ocbf.defaults import (
    CROWD_ENABLE_VISIBLE_HOCBF_DEFAULT,
...
def _route_xy():
    return crowd_narrow._waypoint_xy_array(ROUTE_WAYPOINTS)


def _polyline_cumulative_lengths(path_xy):
    path_xy = crowd_narrow._waypoint_xy_array(path_xy)
    cum = [0.0]
    total = 0.0
...
def _rows_overlap(candidate_xy, candidate_r, existing_rows, margin):
    return crowd_narrow._rows_overlap(candidate_xy, candidate_r, existing_rows, margin)
...
def _simulate_route_forced_event_nominal(
    *,
    ...
    for step in range(n_steps + 1):
        ...
        occluded = crowd_narrow._disc_occludes_target(
            ego_xy,
            hxy,
            occluder_xy,
            occluder_radius,
...
def _background_candidate_valid(row, existing_rows, min_route_clearance, start_clearance):
    ...
    center = row[:2]
    radius = float(row[2])
    if crowd_narrow._point_polyline_distance(center, ROUTE_XY) < float(min_route_clearance):
        return False
...
def _sample_hidden_for_event(
    *,
    ...
    occ_dir = crowd_narrow._safe_normalize(np.asarray(occ_xy, dtype=float).reshape(2,) - np.asarray(observer_xy, dtype=float).reshape(2,))
    if occ_dir is None:
        return None
...
def _build_route_forced_emergence_scenario(
    *,
                    ...
                    for along_jitter in along_candidates:
                        for lead_distance in lead_candidates:
                            for occ_radius in radius_candidates:
                                ...
                                away_dir = crowd_narrow._safe_normalize(side_sign * seg["lateral"])
                                if away_dir is None:
    ...
    bg_target = (
        max(int(preferred_bg_target), max(0, int(n_rand) - hidden_total_actual))
        if bool(rand_obs)
        else 0
    )
...
def run_crowd_scenario(
    ...
    occ_enable_visible_hocbf=CROWD_ENABLE_VISIBLE_HOCBF_DEFAULT,
...
def main(argv=None):
    ...
    parser.add_argument("--uni-allow-reverse", type=crowd_narrow._str2bool, nargs="?", const=True, default=None)
    parser.add_argument("--uni-forward-only", type=crowd_narrow._str2bool, nargs="?", const=True, default=False)
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=True,
        default=CROWD_ENABLE_VISIBLE_HOCBF_DEFAULT,
